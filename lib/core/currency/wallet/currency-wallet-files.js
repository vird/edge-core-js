 ...
 */
function loadFiatFile(input, folder) {
  const walletId = input.props.id
 ...
 */
function loadNameFile(input, folder) {
  const walletId = input.props.id
  const { dispatch } = input.props
  ...
  return folder
    ...
    .catch(async e => {
      ...
      const fullWalletInfo = input.props.selfState.walletInfo
 ...
 */
async function loadTxFileNames(input, folder) {
  const walletId = input.props.id
  const { dispatch, state } = input.props
 ...
 */
function loadAddressFiles(input, folder) {
...
export function setupNewTxMetadata(input, tx) {
  const walletId = input.props.id
  const { dispatch, state } = input.props
  ...
  const fiatCurrency = input.props.selfState.fiat || 'iso:USD'
